...
use Rapid\Cache\AuthorizationCache;
...
use Rapid\Exception\ConnectionException;
use Rapid\Handler\IHandler;
use Rapid\Rest\ApiContext;
use Rapid\Security\Cipher;
...
class OAuthTokenCredential extends ResourceModel implements ICredential
{
     ...
     */
    public static $AUTH_HANDLER = 'Rapid\Handler\OauthHandler';
     ...
     * Private Variable
     *
     * @var \Rapid\Core\LoggingManager $logger
     */
    private $logger;
    ...
    {
        $httpConfig = new HttpConfig(null, 'POST', $config);
    ...
    private function generateAccessToken($config, $refreshToken = null)
    {
        ...
        if ($response == null || !isset($response["access_token"]) || !isset($response["expires_in"])) {
            ...
            throw new ConnectionException(null, "Could not generate new Access token. Invalid response from server: ");
        } else {
